...
from src.config.channels import EN_HUB_CHANNEL_ID, LEADERBOARD_CHANNEL_ID, NL_HUB_CHANNEL_ID
...
class StartHereView(discord.ui.View):
    ...
    async def start_playing(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_message(
            ...
            "Word Chain -> go to <#1481745881123520573> and type `/games wordchain_start`\n\n"
            "Earn beans by playing. Check `/cafe beans` for your balance.\n"
            f"See the leaderboard in <#{LEADERBOARD_CHANNEL_ID}>.",
            ephemeral=True,
        )
...
def build_en_hub_embed() -> discord.Embed:
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Word Chain** | <#1481745881123520573>\n"
            "Build a chain where each word starts with the last letter of the previous one.\n"
            "Start: `/games wordchain_start`\n\n"
            "**Economy**\n"
            "Earn beans by playing and spend them in `/shop`.\n"
            "Claim daily beans with `/cafe daily` in <#1481764812248842280>.\n\n"
            f"**Leaderboard**: <#{LEADERBOARD_CHANNEL_ID}>"
        ),
    )
...
class BeginHierView(discord.ui.View):
    ...
    async def begin_spelen(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        await interaction.response.send_message(
            ...
            "Woordketting -> ga naar <#1482763114842816765> en typ `/games woordketting_start`\n\n"
            "Verdien bonen door te spelen. Check `/cafe bonen` voor je saldo.\n"
            f"Bekijk het scorebord in <#{LEADERBOARD_CHANNEL_ID}>.",
            ephemeral=True,
        )
...
def build_nl_hub_embed() -> discord.Embed:
    embed = discord.Embed(
        ...
        description=(
            ...
            "**Woordketting** | <#1482763114842816765>\n"
            "Bouw een ketting waarin elk woord begint met de laatste letter van het vorige woord.\n"
            ...
            f"**Scorebord**: <#{LEADERBOARD_CHANNEL_ID}>"
        ),
    )
