...
class DutchWordChainGame:
    """
    Dutch Word Chain — identical logic to WordChainGame but uses words_nl.txt
    ...
    def _new_state(self) -> dict[str, Any]:
        return {
            ...
            "last_status": "Ronde gestart. Wacht op het eerste woord…",
            "last_milestone": 0,
        }
    ...
    def _build_status_embed(self, *, state: dict[str, Any], channel_id: int) -> discord.Embed:
        ...
        next_letter_text = next_letter.upper() if next_letter else "ELKE"

        embed = discord.Embed(
            title="🔤 Woordketting — Live Ronde",
            description=(
                "**Spelregels**\n"
                "• Stuur een Nederlands woord (alleen letters)\n"
                "• Het volgende woord moet beginnen met de **laatste letter** van het vorige woord\n"
                "• Geen herhalingen\n"
                "• Eindigt direct bij een fout\n\n"
                "**Typ je eerste woord:**"
            ),
        )
        embed.add_field(name="Laatste woord", value=f"**{last_word}**", inline=True)
        embed.add_field(name="Volgende letter", value=f"**{next_letter_text}**", inline=True)
        embed.add_field(name="Beurten", value=str(turn), inline=True)
        embed.add_field(name="Status", value=last_status, inline=False)
        embed.set_footer(text=f"Kanaal: {channel_id}")
        return embed
    ...
    async def _edit_status_embed(
        ...
        *,
        ...
        if not msg_id or not hasattr(channel, "fetch_message"):
            return
        ...
        *,
        ...
        try:
            await channel.send(  # type: ignore[attr-defined]
                f"🎉 **{turn} beurten!** Goed bezig <@{actor_id}>!"
            )
        except Exception:
            pass
        ...
        async with self._get_channel_lock(channel_id):
            ...
            if not self._wordlist.is_word(word):
                await self._finish_round(
                    ...
                    reason="Geen geldig woordenboekwoord",
                    state=state,
                )
                return True
            ...
            if unique_players >= 2 and last_player_id == author_id_str:
                await self._finish_round(
                    ...
                    channel_id=channel_id,
                    ended_by_discord_id=message.author.id,
                    reason="Twee keer achter elkaar gespeeld",
                    state=state,
                )
                return True
            ...
            if required_letter and word[0] != required_letter:
                await self._finish_round(
                    ...
                    channel_id=channel_id,
                    ended_by_discord_id=message.author.id,
                    reason=f"Verkeerde beginletter (nodig: **{required_letter}**)",
                    state=state,
                )
                return True
            ...
            state["last_status"] = f"✅ Geaccepteerd: **{word}** (door <@{message.author.id}>)"
    ...
    async def _finish_round(
        ...
        *,
        ...
        state["last_status"] = f"🛑 Ronde gestopt — {reason}"
        await self._edit_status_embed(channel=channel, channel_id=channel_id, state=state, force=True)
                ...
                try:
                    ...
                    if beans > 0:
                        await self._economy.award_beans_discord(
                            user_id=discord_id,
                            amount=beans,
                            reason="Woordketting uitbetaling",
                            game_key=self.key,
                            display_name=None,
                            metadata=None,
                            guild_id=GUILD_NL,
                    ...
                    streak_lines.append(f"<@{discord_id}> — **{beans}** bonen (reeks {streak})")
                except Exception as e:
                    streak_lines.append(f"<@{discord_id}> — **{beans}** (uitbetaling mislukt)")
                    print(f"[WoordKetting fout] discord_id={discord_id} err={e!r}")

        embed = discord.Embed(
            title="🛑 Woordketting Voorbij!",
            description=f"Gestopt door <@{ended_by_discord_id}> — {reason}",
        )
        embed.add_field(name="Laatste woord", value=f"**{last_word or '—'}**", inline=True)
        embed.add_field(name="Beurten", value=str(total_turns), inline=True)
        embed.add_field(
            name="🔥 Reeksen (bonen verdiend)",
            value="\n".join(streak_lines) if streak_lines else "(geen)",
            inline=False,
        )
        embed.set_footer(text="Druk op Opnieuw spelen om direct een nieuwe ronde te starten.")

        await channel.send(embed=embed, view=SpeelOpnieuwView(game=self, channel_id=channel_id))  # type: ignore[attr-defined]
...
class SpeelOpnieuwView(discord.ui.View):
    def __init__(self, *, game: DutchWordChainGame, channel_id: int, timeout: float = 900.0) -> None:
        super().__init__(timeout=timeout)
        self._game = game
        self._channel_id = channel_id

    @discord.ui.button(label="Opnieuw spelen", style=discord.ButtonStyle.success, emoji="🔁")
    async def speel_opnieuw(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        embed = discord.Embed(
            title="🔤 Woordketting — Live Ronde",
