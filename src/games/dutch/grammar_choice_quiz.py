...
class _ChoiceView(discord.ui.View):
    def __init__(self, *, game: "GrammarChoiceQuizGame", rs: _RoundState, gs: _GameState) -> None:
...
class _RestartView(discord.ui.View):
    def __init__(self, *, game: "GrammarChoiceQuizGame") -> None:
        super().__init__(timeout=300)
        self._game = game

    @discord.ui.button(label="Opnieuw spelen", style=discord.ButtonStyle.primary, custom_id="grammar:restart")
    async def restart(self, interaction: discord.Interaction, button: discord.ui.Button) -> None:
        ...
        await self._game.start(channel)
...
class GrammarChoiceQuizGame:
    ...
    def _question_embed(self, rs: _RoundState, gs: _GameState, seconds_left: int) -> discord.Embed:
        ...
        total = min(MAX_QUESTIONS, len(gs.questions))
        embed = discord.Embed(
    ...
    def _final_embed(self, gs: _GameState) -> discord.Embed:
        ...
        questions_answered = gs.total_answers
        wrong = max(0, gs.total_answers - gs.correct_answers)
    ...
    async def _run(self, channel: discord.TextChannel, gs: _GameState) -> None:
        try:
            questions = random.sample(gs.questions, k=min(MAX_QUESTIONS, len(gs.questions)))
            for idx, q in enumerate(questions, start=1):
                if channel.id not in self._active:
                    return
                rs = _RoundState(question=q, number=idx)
                view = _ChoiceView(game=self, rs=rs, gs=gs)
