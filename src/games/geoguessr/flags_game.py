...
class GeoFlagsGame:
    """
    Geo Flags — streak mode (Word Chain style), per-user panel in a shared channel.

    Flow:
      - /geoguessr flags_start creates your panel
      - You type country names in the channel
      - Correct:
          - +2 beans per correct
          - +3 bonus beans every 10 streak (10, 20, 30, ...)
          - streak continues with a NEW flag immediately (panel updates)
      - Wrong:
    ...
    def _beans_per_correct(self) -> int:
        ...
        try:
            return int(self._rewards.amount(RewardKey.GEO_FLAGS_CORRECT))
        except Exception:
            return 2

    def _bonus_every(self) -> int:
        try:
            return int(self._rewards.amount(RewardKey.GEO_FLAGS_BONUS_EVERY))
        except Exception:
            return BONUS_EVERY

    def _bonus_amount(self) -> int:
        try:
            return int(self._rewards.amount(RewardKey.GEO_FLAGS_BONUS_AMOUNT))
        except Exception:
            return BONUS_BEANS
    ...
    def _build_panel_embed(self, *, player_id: int, p: _Progress) -> discord.Embed:
        status_line = "Type the country name in this channel."
        if p.finished:
            status_line = "🛑 Streak ended. Press Play again below."

        embed = discord.Embed(
            title="🏁 GeoGuessr — Flags (Streak)",
            description=(
                f"**Flag:** {p.prompt}\n"
                f"**Streak:** **{p.streak}**\n\n"
                f"{status_line}"
        ...
        embed.set_thumbnail(url=AssetLinks.GEO_FLAGS_ICON)
    ...
    async def _send_streak_end(
        ...
        *,
        ...
        class PlayAgainView(discord.ui.View):
            def __init__(self, outer: "GeoFlagsGame") -> None:
            ...
            async def play_again(  # type: ignore[override]
                ...
                await interaction.followup.send("✅ New flag posted — streak restarted!", ephemeral=True)

        embed = discord.Embed(
            title="🛑 Geo Flags — Streak ended",
            description=(
    ...
    async def handle_discord_message(self, message: discord.Message) -> bool:
        ...
        async with self._lock_for_channel(channel_id):
            ...
            # Only consume messages for users who have an active panel
            if pid not in players:
                return False

            p = self._progress_from_dict(players.get(pid, {}))

            # If finished, consume to reduce noise
            if p.finished:
                return True

            # Ensure panel exists (recover if deleted)
            ...
            if is_correct:
                ...
                await self._economy.award_beans_discord(
                    user_id=player_id,
                    amount=beans,
                    reason="Geo Flags streak correct",
                    game_key=self.key,
                ...
                # swap to a new flag (avoid immediate repeats when possible)
                item = self._pick_item()
                # tiny best-effort re-roll
                for _ in range(4):
                ...
                # Optional: quick public ping on milestones (lightweight)
                if bonus > 0:
                    try:
                        await message.channel.send(
                            f"🎉 <@{player_id}> hit a **{p.streak}** flag streak! **+{bonus} bonus beans**!"
                        )
                    except (discord.Forbidden, discord.HTTPException):
                        pass
            ...
            try:
                ...
                logger.exception("GeoFlags: failed recording loss")
